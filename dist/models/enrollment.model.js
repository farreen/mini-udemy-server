...
export const createEnrollment = async (studentId, courseId) => {
    ...
    const [existing] = await db.execute("SELECT * FROM enrollments WHERE student_id = ? AND course_id = ?", [studentId, courseId]);
...
export const getEnrollmentsByCourse = async (courseId) => {
    const [rows] = await db.execute(`SELECT e.*, u.name as student_name, u.email as student_email
     FROM enrollments e
...
export const checkEnrollment = async (studentId, courseId) => {
    const [rows] = await db.execute("SELECT * FROM enrollments WHERE student_id = ? AND course_id = ?", [studentId, courseId]);
